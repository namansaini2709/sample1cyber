...
def setup_db():
    ...
    users = [
        ("Alice Smith", "alice@example.com", password, "VIP Customer"),
        ("Bob Jones", "bob@example.com", password, "Frequent returns"),
        ("Charlie Brown", "charlie@example.com", password, "Regular"),
        ("Diana Prince", "diana@example.com", password, "High value cart limit"),
        ("Eve Adams", "eve@example.com", password, "Loyalty program"),
        ("Frank Castle", "frank@example.com", password, "Watchlist"),
        ("Grace Hopper", "grace@example.com", password, "Tech Lead"),
        ("Henry Ford", "henry@example.com", password, "Bulk ordering"),
        ("Ivy Carter", "ivy@example.com", password, "Standard"),
        ("Jack Sparrow", "jack@example.com", password, "Flagged for fraud")
    ]
...
# Use the getpass module to safely prompt for passwords
import getpass
username = getpass.getuser()
# Use the secrets module to safely generate passwords
password = secrets.token_hex(16)
print(f"User {username} password has been generated as {password}.")
